...
export default function NavigationPanel({ user }) {
    ...
    const onMoveToHomePageCallback = () => navigate(homepageURL);

    return (
        <Navbar bg="light">
            <Container fluid>
                <Navbar.Brand as={Link} to={homepageURL} className="ms-4">
                    <i className="bi bi-twitter" style={{ fontSize: 30, color: "dodgerblue" }}></i>
                </Navbar.Brand>
                <Navbar.Collapse className="justify-content-end">
                    <Image onClick={onMoveToHomePageCallback}
                    ...
                    <Button variant="link" className="nav-panel-text"
                        onClick={onMoveToHomePageCallback}>
